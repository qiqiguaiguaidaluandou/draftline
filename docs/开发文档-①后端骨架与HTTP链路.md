...
**不变量**：`TZHJ.Core` 的网关契约（`IAuthGateway`/`IDataGateway`/`ISubmitGateway`/`IConfigGateway`）
...
```
┌─ 客户端 (TZHJ.App + TZHJ.Infrastructure) ──────────────┐
│  UI / ViewModel  (不动)                                  │
│  ILocalBatchStore = LocalBatchStore  (不动)              │
│  IAuthGateway/IDataGateway/ISubmitGateway/IConfigGateway │
│    ├─ AddTzhjMockInfrastructure  → Mock*Gateway (保留=离线模式) │
│    └─ AddTzhjHttpInfrastructure  → Http*Gateway  ★本期新增      │
└───────────────────────────────┬────────────────────────┘
                                 │  HTTP (JSON + 图纸流)
┌─ 后端 (TZHJ.Gateway, ASP.NET Core 无状态) ─┴────────────┐
...
```
TZHJ.sln
├─ src/TZHJ.Core            (不动 + 新增 Contracts/Http 共享 wire DTO)
├─ src/TZHJ.Infrastructure  (新增 Gateways/Http/ 四个 Http*Gateway + HttpOptions)
├─ src/TZHJ.App             (改 App.xaml.cs 的 DI 分支 + appsettings 加 Http 节)
├─ src/TZHJ.SampleData      (不动)
└─ src/TZHJ.Gateway         ★新增：ASP.NET Core 无状态网关
...
**共享 wire DTO 放 `TZHJ.Core/Contracts/Http/`**：客户端与后端都引用 `TZHJ.Core`，同一份定义，
...
| D4 | 配置/审计存储 | **骨架内存/JSON，DB 后置** | `IConfigStore` 先从 `TZHJ.Core` 默认 schema 种子；`IAuditStore` 先内存。**审计表结构本期定下**（追溯刚需） |
...
`IConfigStore.GetAsync(empId) → ClientConfig`：骨架实现从 `TZHJ.Core` 的 `CollectionSchedules.*` +
...
**新增 `src/TZHJ.Infrastructure/Gateways/Http/`**：`HttpAuthGateway`/`HttpConfigGateway`/
`HttpDataGateway`（含 §4.2 下载拼装）/`HttpSubmitGateway`，各自实现对应 `TZHJ.Core` 契约。

**新增 `HttpOptions`**（绑定 appsettings `Http` 节）：`BaseUrl`、`TimeoutSeconds`、（可选）重试次数。

**新增 `AddTzhjHttpInfrastructure(this IServiceCollection, HttpOptions)`**（与 `AddTzhjMockInfrastructure` 并存）：
...
**`App.xaml.cs`**：把现在 `else` 分支的 `throw`（第 42 行）换成读 `Http` 节 + `services.AddTzhjHttpInfrastructure(http)`；
...
1. 加 `TZHJ.Gateway` 工程入解；`TZHJ.Core/Contracts/Http/` 加 `FetchResponse`/`FetchRowDto`/`DrawingMeta`。
...
5. `AddTzhjHttpInfrastructure` + `App.xaml.cs` 切换 + `appsettings` Http 节。
6. 端到端联调。
...
> 1. **令牌持有位置**：§7 原写"从 `ISession` 取 token"。但 `ISession` 在 `TZHJ.App`，而令牌处理器 `AuthTokenHandler`
>    在 `TZHJ.Infrastructure`，不能反向依赖 App；且令牌须在"登录响应到手"与"取配置(受保护)"之间就绪、早于
