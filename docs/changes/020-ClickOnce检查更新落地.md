...
- 新增 `src/Draftline.App/Services/UpdateService.cs`：
...
- 改 `src/Draftline.App/ViewModels/SettingsViewModel.cs`：`CheckUpdate()` 换成四分支——未部署（开发/直跑）提示无更新通道；本次刚更新过报「已更新至 vX」；无重激活地址提示手动重启；正常则确认后重启触发更新。构造注入 `IUpdateService`。
- 改 `src/Draftline.App/App.xaml.cs`：注册 `IUpdateService`；登录后若为安装/更新后首次运行（`IsFirstRun`），Toast 提示「客户端已更新至 vX」。

部署/发布整体流程另见 `docs/开发文档-⑨部署与发布(Linux后端+ClickOnce客户端).md`。

## 验证

- Linux 上受 WPF 平台限制，需加 `-p:EnableWindowsTargeting=true` 才能编译；已如此编译 `Draftline.App`：**成功，0 warning 0 error**。
