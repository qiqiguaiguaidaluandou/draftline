...
{
    public static void MapDraftlineApi(this WebApplication app)
    {
        app.MapGet("/healthz", () => Results.Ok("ok"));

        // 登录（不需令牌）→ 校验本地凭证、签发 JWT。权限由管理员显式维护（Deny-All 白名单，无自动放权）。
        app.MapPost("/api/auth/login", async (LoginRequest req, IAuthService auth, DraftlineDbContext db, HttpContext ctx, CancellationToken ct) =>
        {
        ...
        api.MapPost("/auth/change-password", async (ChangePasswordRequest req, IAuthService auth, DraftlineDbContext db, HttpContext ctx, CancellationToken ct) =>
        {
        ...
        // 1. 同步清单 (基于权限白名单过滤)
        api.MapGet("/sync/catalog", async (HttpContext ctx, IPermissionService perm, DraftlineDbContext db, IServerBatchStore store, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
        ...
        api.MapPost("/batch/update-row", async ([FromBody] UpdateRowRequest req, HttpContext ctx, IPermissionService perm, IServerBatchStore store, DraftlineDbContext db, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
        ...
        // 4. 异常挂起
        api.MapPost("/batch/suspend-exception", async ([FromBody] SuspendExceptionRequest req, HttpContext ctx, IPermissionService perm, DraftlineDbContext db, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
            if (!await perm.HasAccessAsync(empId, req.Flow, req.GroupName, ct)) return Results.Forbid();
        ...
        // 5. 异常池查询 (组内共享视野)
        api.MapGet("/sync/exceptions", async (HttpContext ctx, IPermissionService perm, DraftlineDbContext db, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
            var grants = await perm.GetGrantsAsync(empId, ct);
        ...
        api.MapPost("/batch/refetch-drawing", async ([FromBody] RefetchDrawingRequest req, HttpContext ctx, IPermissionService perm, PlmClient plm, IServerBatchStore store, DraftlineDbContext db, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
            if (!await perm.HasAccessAsync(empId, req.Flow, req.GroupName, ct)) return Results.Forbid();
        ...
        api.MapPost("/submit", async ([FromBody] SubmitRequest req, HttpContext ctx, IPermissionService perm, ISubmitSink sink, DraftlineDbContext db, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
            if (!await perm.HasAccessAsync(empId, req.Flow, req.GroupName, ct)) return Results.Forbid();
        ...
        api.MapGet("/oplog/mine", async (HttpContext ctx, DraftlineDbContext db, CancellationToken ct) =>
        {
            var empId = ctx.GetEmployeeId();
        ...
        api.MapGet("/audit/exists", async (string flow, string windowStart, string windowEnd, HttpContext ctx, DraftlineDbContext db, CancellationToken ct) =>
        {
