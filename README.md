...
Draftline.sln
src/
  Draftline.Core/            领域模型 + 网关契约 + 字段 schema + 本地路径/批次窗口规则（跨平台，无 WPF）
  ...
  Draftline.Infrastructure/  契约实现（跨平台，无 WPF）
    ...
    DependencyInjection.cs   AddDraftlineHttpInfrastructure(...)  ← 客户端唯一链路注册入口
  Draftline.Gateway/         无状态后端网关（net8.0，ASP.NET Core）：端点 + 防腐层(FakeDataSource 占位) + 存储
  Draftline.App/             WPF 客户端（net8.0-windows，仅此工程需 Windows）：MVVM + DI
...
```powershell
dotnet run --project src/Draftline.Gateway     # 1) 起后端网关（默认 http://localhost:8080）
dotnet run --project src/Draftline.App         # 2) 起客户端（连 appsettings.json 的 Http:BaseUrl）
```

登录：工号任意非空（如 `10086`）、密码任意（填 `fail` 可演示登录失败）。本地根目录见
`src/Draftline.App/appsettings.json` 的 `Http:LocalRoot`。登录后会自动补拉，待处理批次即从后端取下。

**Linux / macOS / CI（仅验证编译，不能运行 WPF）：**

```bash
dotnet build Draftline.sln -p:EnableWindowsTargeting=true
```
...
1. 把 `Draftline.Gateway/AntiCorruption/FakeDataSource`（占位造数）替换为真正调 EBS / PLM / SRM 的实现。
...
按方案选型走 **ClickOnce**。发布配置在 `src/Draftline.App/Properties/PublishProfiles/ClickOnceProfile.pubxml`
...
或在 Visual Studio 右键 `Draftline.App` → 发布 → 选 `ClickOnceProfile`。

上线前要改 `.pubxml` 里两处占位：`PublishUrl` / `InstallUrl` 指向真实发布目标（文件共享
`\\server\share\Draftline` 或 https 站点）。更新策略已设为 **启动前同步检查**（`UpdateMode=Foreground`）——
